...
修改 `config/mysql.go` 和 `config/redis.go` 中的配置信息
...
### 多轮评审
- `POST /round/create` - 创建评审轮次
- `GET /round/list` - 获取评审轮次列表
- `GET /round/detail/:id` - 获取轮次详情
- `PUT /round/status` - 更新轮次状态
- `POST /round/assign` - 分配评审任务
- `GET /round/reviews` - 获取轮次评审任务
- `GET /round/tasks` - 获取评审员任务
- `POST /round/submit` - 提交评审评分
- `GET /round/progress` - 查询轮次进度
- `GET /round/results` - 获取轮次评审结果
...
#### GET /round/tasks - 获取评审员任务
```
Query参数: round_id=1&reviewer_id=2
```
...
| status | varchar | 状态(pending/ongoing/completed) |
...
│   └── review_round_service.go # 多轮评审服务
└── api/handler/
    ...
    └── review_round_handler.go # 多轮评审接口
├── web/
│   ├── admin/index.html       # 后台管理面板
│   └── personal/index.html    # 个人中心页面
```
